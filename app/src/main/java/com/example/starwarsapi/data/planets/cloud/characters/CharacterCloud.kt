...
interface CharacterCloud {
    ...
    ): CharacterCloud
    {
        override fun <T> map(mapper: Mapper<T>): T {
           return mapper.map(name,birthYear,url)
        }
    }
    ...
    interface Mapper<T> {
        ...
        {
            fun create(arg:B):T
            class Base(private val urlIdMapper: UrlIdMapper): Factory<Mapper<CharacterCache>, Int>
            {
                override fun create( arg: Int): Mapper<CharacterCache> {
                    return BaseToCharacterCache(urlIdMapper,arg)
                }
