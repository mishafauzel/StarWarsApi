...
interface PlanetsInteractor {
    ...
    suspend fun getListOfPlanetsByPage(page: PagerData, successful: (PlanetsUi) -> Unit,atFinish:()->Unit)
    ...
    class Base(
        ...
        override suspend fun getListOfPlanetsByPage(
            ...
            {
                dispatchers.changeToUI { successful.invoke(domainEx.map(mapperDomainExceptionMapperToListOfItemsUIMapper.create(pageData))) }
            }
            finally {
